...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0"><i class="fas fa-edit mr-2 text-primary"></i>Edit Expense</h1>
      </div>
      <div class="col-sm-6">
            ...
            <div class="col-md-6">
              <div class="form-group">
                    ...
                    <div class="d-flex align-items-center justify-content-between">
                      <div class="d-flex align-items-center">
                        @if(in_array($ext, ['jpg', 'jpeg', 'png']))
                          <img src="{{ asset('storage/' . $expense->bill_path) }}" alt="Current Bill" class="img-thumbnail mr-2" style="height:48px;width:64px;object-fit:cover;">
                        @else
                          <span class="btn btn-sm btn-danger mr-2" style="pointer-events:none;"><i class="fas fa-file-pdf"></i></span>
                        @endif
                        <div>
                          <div class="font-weight-bold" style="font-size:.83rem;">Current Bill</div>
                          <small class="text-muted">{{ $expense->bill_original_name ?? strtoupper($ext) . ' file' }}</small>
                        </div>
                      </div>
                      <a href="{{ asset('storage/' . $expense->bill_path) }}" target="_blank" class="btn btn-outline-primary btn-xs">
                ...
                <div class="custom-file">
                  ...
                  <label class="custom-file-label" for="bill">{{ $expense->bill_path ? 'Replace file...' : 'Choose file...' }}</label>
                </div>
                <small class="form-text text-muted">
                  <i class="fas fa-info-circle mr-1"></i> Supported: PDF, JPG, PNG. @if($expense->bill_path) Leave blank to keep current bill. @endif
                </small>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" name="description" id="description" rows="4" placeholder="Add a short description of the expense, items purchased, or purpose.">{{ old('description', $expense->description) }}</textarea>
            ...
            <div class="col-md-6">
              <div class="form-group">
                ...
                <textarea class="form-control @error('note') is-invalid @enderror" name="note" id="note" rows="4" placeholder="Enter the key reason for this expense or any important internal note.">{{ old('note', $expense->note) }}</textarea>
