...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0"><i class="fas fa-plus-circle mr-2 text-primary"></i>Create Project</h1>
      </div>
      <div class="col-sm-6">
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                ...
                <input name="name" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror" placeholder="Enter project name" required>
          ...
          <div class="row">
              ...
              <div class="form-group">
                ...
                <select id="user_ids" name="user_ids[]" class="form-control select2 project-user-select @error('user_ids') is-invalid @enderror @error('user_ids.*') is-invalid @enderror" multiple data-placeholder="Select one or more users">
                  @foreach($users as $user)
                    <option value="{{ $user->id }}" {{ in_array($user->id, old('user_ids', [])) ? 'selected' : '' }}>{{ $user->name }}</option>
                  @endforeach
                </select>
            ...
            <div class="col-md-12">
              <div class="form-group">
                ...
                <textarea name="note" class="form-control" rows="3" placeholder="Optional project note...">{{ old('note') }}</textarea>
              </div>
            </div>
          </div>
      </div>
      <div class="card-footer">
        <button class="btn btn-primary"><i class="fas fa-save mr-1"></i>Create Project</button>
